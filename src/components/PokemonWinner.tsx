...
export const PokemonWinner = (props: any) => {
  ...
  const opacity = interpolate(
    frame,
    [0, config.durationInFrames * 0.3],
    [0, 1]
  );

  function announcePokemonWinnerSvg(
    pokemon1Hp: number,
    pokemon2Hp: number,
    url1: string,
    url2: string,
  ...
  const [pokeUrl, pokeName] = announcePokemonWinnerSvg(
    props.pokemon1Skills.hp,
    props.pokemon2Skills.hp,
    props.url1,
    props.url2,
    props.name1,
    props.name2
  ...
  return (
    <div
      className="flex flex-col w-full items-center justify-center p-8"
      style={{ opacity }}
    >

...
export function SkillsTable(props: SkillsTableProps): JSX.Element {
    ...
    const value = interpolate(
        frame,
        [0, ANIMATION_DECREASE_HP * config.durationInFrames, config.durationInFrames],
        [1, 1.5, 1]
    );

    function calculateHp(initialHp: number | undefined, skillsHp: number, shouldUseDecreaseAnimation: boolean | undefined, frame: number, durationInFrames: number): number {
        // Pokemonpresentation
        if (!initialHp) {
            return skillsHp;
        }
    ...
    return (
        ...
        >
            ...
            <p
                style={{
                    ...(props.shouldUseDecreaseAnimation && { transform: `scale(${value})` }),
                }}
            >
                {'HP'}: {calculateHp(props.initialHp, props.skills.hp, props.shouldUseDecreaseAnimation, frame, config.durationInFrames)}
            </p>
            <p>
